...
├─ samples/       … サンプルゲーム(スイカゲーム: 純C#ロジック + Godot プロジェクト)
└─ tests/      … テスト
```
...
例外として、`samples/*/assets/fonts/` に同梱している
[Noto Color Emoji](https://github.com/googlefonts/noto-emoji) のサブセットは
**SIL Open Font License 1.1**(各ディレクトリの `OFL.txt` 参照)であり、
本リポジトリの MIT ライセンスは適用されません。

また、`games/MessBreak/` 以下(ゲーム本体のコード・ドキュメント・アセット)は
**ソース公開・全権利留保**([games/MessBreak/LICENSE](games/MessBreak/LICENSE) 参照)であり、
MIT ライセンスは適用されません。
