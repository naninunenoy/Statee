...
- 論理は `_PhysicsProcess`(60Hz)で 1 Tick。エージェントのプレイ経路は
  freeze + `tick` コマンド(`--arg frames=N,input=forward+shoot`)
- 人型モデルは `assets/voxels/*.voxel.txt` を voxcee で GLB 化した
  `assets/models/*.glb`。定義を直したら `dotnet run --project tools/voxcee/Voxcee -- render -i …`
  で再生成する
