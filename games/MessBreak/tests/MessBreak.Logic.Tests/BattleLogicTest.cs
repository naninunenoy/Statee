...
public class BattleLogicTest
{
    ...
    /// <summary>敵が攻撃してこない(アグロしない)状態で、目の前に敵がいる配置。</summary>
    private static BattleConfig AdjacentPassiveEnemy() =>
        new()
        {
            PlayerSpawn = new Vector2(100f, 90f),
            EnemySpawn = new Vector2(115f, 90f),
            EnemyAggroRange = 0f,
        };

    [Fact]
    public void Tick_攻撃入力_Attack状態になる()
    {
        var logic = Create();

        logic.Tick(new TickInput(Vector2.Zero, Attack: true));

        logic.PlayerAction.ShouldBe(PlayerAction.Attack);
    }

    [Fact]
    public void Tick_攻撃中の移動入力_移動しない()
    {
        var logic = Create();
        logic.Tick(new TickInput(Vector2.Zero, Attack: true));
        var before = logic.PlayerPos;
    ...
    [Fact]
    public void Tick_正面の敵に攻撃_1回分のダメージを与える()
    {
        var logic = Create(AdjacentPassiveEnemy());
        logic.Tick(new TickInput(new Vector2(1f, 0f))); // 敵の方を向く

        logic.Tick(new TickInput(Vector2.Zero, Attack: true));
        TickUntil(logic, () => logic.PlayerAction == PlayerAction.Free);

        logic.EnemyHp.ShouldBe(logic.Config.EnemyMaxHp - logic.Config.AttackDamage);
    }

    [Fact]
    public void Tick_射程外の敵に攻撃_ダメージを与えない()
    {
        var logic = Create(); // 既定配置は 160 離れている
        logic.Tick(new TickInput(new Vector2(1f, 0f)));
    ...
    {
        var logic = Create(AdjacentPassiveEnemy() with { EnemyMaxHp = 1 });
        logic.Tick(new TickInput(new Vector2(1f, 0f)));

        logic.Tick(new TickInput(Vector2.Zero, Attack: true));
        TickUntil(logic, () => logic.EnemyHp == 0);
    ...
    public void Tick_Victory後_状態が進まない()
    {
        ...
        logic.Tick(new TickInput(new Vector2(1f, 0f)));
        logic.Tick(new TickInput(Vector2.Zero, Attack: true));
        TickUntil(logic, () => logic.Phase == BattlePhase.Victory);
