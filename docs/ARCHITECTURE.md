...
## サンプルゲーム:RaidBoss(2人協力ボス戦・決定論ロックステップ、全フェーズ完了 ✅)

Syncee がまだ実証していなかった「常時・高頻度・複数エンティティ」のリアルタイム同期
(決定論ロックステップ)を実証する5つ目のサンプル(D-053)。リバーシ(ターン制・低頻度・
即時確定)とは異なる同期パターンを意図的に選んだ。規模は最小(プレイヤー2人+ボス1体)に絞る。

- **ローカル版**: 固定Tick駆動で両プレイヤーのAttack/Idleを同時適用、ボスは一定周期で
  プレイヤーを交互反撃。ボス撃破でVictory、両プレイヤー戦闘不能でDefeatへ遷移。
  乱数を使わず攻撃対象を選ぶため同一シード・同一入力列で結果が完全一致する
  (`RaidBoss.Logic`、D-011/D-049 と同型の決定論)
- **ネットワーク対戦化**: 決定論ロックステップ(D-054)。Syncee コアに
  ...
  サーバ(純C# `RaidBoss.Server`)は物理・当たり判定を持たず、両クライアントのTick入力が
  揃うまで待って確定・配布するのみ。各クライアントは確認応答を待たず単調増加するTick番号で
  先行送信できる。実サーバ+2クライアントのマルチインスタンスE2E(D-051の語彙)で、
  全インスタンスが同一Tickで完全に同じ状態に収束することを確認済み
...
- Godot 物理エンジンを使った当たり判定(決定論を壊すリスクがあるため、当面は
  `RaidBoss.Logic` 内の純粋計算のみ)
- サーバ側の接続管理・ブロードキャスト配線、および `Program.cs` の起動骨格が
  `Reversi.Server` とほぼ重複している。2例そろったため、`libs/Statee.Godot`(D-047)と
  同じ発想で Syncee 側に共通化する余地がある(検討中。ADR 未起票)
