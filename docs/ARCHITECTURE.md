...
- 全破棄・全再構築(D-035)の限界(Slider のドラッグ破壊・LineEdit の入力消失)が
  ここで証明され、**差分適用(Reconcile)を導入**した(D-061)。
  スライダーはドラッグ中もライブで全体の文字サイズへ反映される
- ドラッグ並び替えの視覚状態(移動中の行=半透明・ドロップ先=ハイライト)も
  宣言に昇格(`ReorderList.DraggingIndex/DropIndex`。D-062)。
  ドラッグ中の見た目まで ui/tree の State で検証できる
- headless E2E で、実入力経路(click/drag/type)による トグル・フィルタ・並び替え・
  編集・削除確認、および **Overlay がモーダル中の背面クリックを遮断すること**を確認済み
- キーボード/ゲームパッドだけでも全操作が可能(D-063)。Godot 標準フォーカスに乗り、
  Declaree は AutoFocus・Reconcile のフォーカス復元・Overlay のフォーカストラップ・
  `focused` の State 公開を担う。並び替えは掴みモード(Ctrl+Space → ↑↓ → Enter/Esc)で、
  マウスドラッグと同じ update/commit/cancel(D-062)に写像される
