...
- **ローカル版**: 固定Tick駆動。Attackは即ダメージではなく弾を発射し
  (`ProjectileTravelTicks`後にボスへ着弾してダメージを与える。D-058)、ボスは一定周期で
  プレイヤーを巡回攻撃。HPが0以下になったプレイヤーは`IncapacitationDuration`(3Tick)だけ
  操作不能になり(弾も発射できない)、明けると`ReviveHp`(最大HPの半分)で復帰する。
  全プレイヤーが同時にHP0以下になったときだけDefeatへ遷移する(ソロプレイでは「全員」が
  自分1人なのでHP0=即Defeatになる。D-057)。ボス撃破でVictory。乱数を使わず攻撃対象・
  着弾を決定論的に処理するため同一シード・同一入力列で結果が完全一致する
  (`RaidBoss.Logic`、D-011/D-049 と同型の決定論)。RaidBoss.Godot は中央にボスを表示し、
  各プレイヤーから発射された弾がボスへ飛んでいく様子をHP・操作不能状態とあわせて
  描画する(D-058。演出用の座標計算のみで、ロジックはRaidBoss.Logicに置いたまま)
...
- Godot 物理エンジンを使った当たり判定(決定論を壊すリスクがあるため、当面は
  `RaidBoss.Logic` 内の純粋計算のみ)。中央にボスを表示し弾を発射する演出化は
  今後のスライスで着手予定
