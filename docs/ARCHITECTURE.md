...
- **ローカル版**: 固定Tick駆動で全プレイヤーのAttack/Idleを同時適用、ボスは一定周期で
  プレイヤーを巡回攻撃。HPが0以下になったプレイヤーは`IncapacitationDuration`(3Tick)だけ
  操作不能になり、明けると`ReviveHp`(最大HPの半分)で復帰する。全プレイヤーが同時に
  HP0以下になったときだけDefeatへ遷移する(ソロプレイでは「全員」が自分1人なので
  HP0=即Defeatになる。D-057)。ボス撃破でVictory。乱数を使わず攻撃対象を選ぶため
  同一シード・同一入力列で結果が完全一致する(`RaidBoss.Logic`、D-011/D-049 と同型の決定論)
