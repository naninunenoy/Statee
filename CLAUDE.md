...
- Godot は **4.7.stable の .NET 版**を使う。標準版は C# を実行できない。
  パスは**環境変数 `GODOT_BIN`** にフルパスで設定する(D-066。設定手順は docs/HANDOVER.md)。
  Windows は `*_console.exe`、macOS は `Godot_mono.app/Contents/MacOS/Godot` を指す
- Godot の headless 実行: `$env:GODOT_BIN --headless --path game/SuikaGame.Godot`(sh は `"$GODOT_BIN"`)。
  初回は `--import` が必要だが、import は完了後にクラッシュするので exit code を無視する(D-016)
- ソリューションは分割されている(D-046): フレームワークは `Statee.slnx`、
  各ゲームは `game/<Name>.slnx`。フレームワーク変更後は `tools/build-all.ps1`
